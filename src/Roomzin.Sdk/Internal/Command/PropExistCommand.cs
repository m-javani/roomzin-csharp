...
{
    /// <summary>
    /// Command builder and parser for PROPEXIST command
    /// </summary>
    public static class PropExistCommand
    {
        /// <summary>
        /// Builds the payload for PROPEXIST command
        /// </summary>
        public static byte[] BuildPropExistPayload(string propertyId)
        {
            var cmdName = "PROPEXIST";
        ...
        /// <summary>
        /// Parses the PROPEXIST response
        /// </summary>
        public static bool ParsePropExistResp(string status, List<Protocol.Field> fields)
        {
            if (status == "SUCCESS")
            {
                if (fields == null || fields.Count == 0)
                    throw RoomzinException.From("RESPONSE_ERROR: missing response data");
