...
{
    /// <summary>
    /// Command builder and parser for PROPROOMEXIST command
    /// </summary>
    public static class PropRoomExistCommand
    {
        /// <summary>
        /// Builds the payload for PROPROOMEXIST command
        /// </summary>
        public static byte[] BuildPropRoomExistPayload(PropRoomExistPayload payload)
        {
            var cmdName = "PROPROOMEXIST";
        ...
        /// <summary>
        /// Parses the PROPROOMEXIST response
        /// </summary>
        public static bool ParsePropRoomExistResp(string status, List<Protocol.Field> fields)
        {
            if (status == "SUCCESS")
            {
                if (fields == null || fields.Count == 0)
                    throw RoomzinException.From("missing response data");
